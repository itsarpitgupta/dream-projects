...
class TemplateMatchingTool(ToolI):
    ...
    def matchTemplateWithSingleObject(self, main_img, temp_img, opt) -> TemplateMatchingOutput:
        ...
        try:
            if main_img is None or temp_img is None or opt is None:
                raise DataValidationException('main_img, temp_img or opt should not be none')
    ...
    def matchTemplateWithMultipleObject(self, main_img, temp_img) -> TemplateMatchingOutput:
        ...
        try:
            ...
            # print(res)
            for pt in zip(*loc[::-1]):
                cv2.rectangle(img_gray, pt, (pt[0] + w, pt[1] + h), (255, 0, 0), 2)
            ...
            if self.display:
                displayImageOutput(main_img=template, main_img_title="Matching Result", result_img=img_gray,
